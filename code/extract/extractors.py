...
# this script has functions that can be called (by mega_collector.py!) to extract linguistic features from texts in EN/GE/RU in conllu-format
# it calls help functions to traverse conllu sentence trees from helpfunctions.py
# the path to the folder of *.conllu should look like: /home/masha/test/data/ru/rltc/essays/prof/
...
from igraph import *
import itertools
from extract.helpfunctions import *
...
## TODO REQUIRES REVISING for German, esp for indefitite pron that refer to a location (-wo)
...
def anysome(tree, lang):
	...
	for w in tree:
		if lang == 'en':
			...
			# TODO see Katinskaya for indefinite constructions like "какой попало", "какой бы то ни было", "непонятно какой, невесть кто"
...
def modpred(tree, lang, mpred_dic):
	...
	if lang == 'de':
		...
		for w in tree:
			...
			# DONE: TODO the below needs to be implemented to account for the cases like
		...
		# DONE TODO what exactly does it get? Make sure it is not getting Unser Ziel ist (cop, not aux!) es, etwas zu machen
	...
	for w in tree:
		if lang == 'en':
			...
			if w[2] in madv_lst and w[3] == 'ADV':
				...
				matches.append(w[2])
		if lang == 'de':
			# TODO implement "and where applicable following the “V*FIN” (finite verb)"
			## TODO check the list! why does it have aber, ehrlich, eigentlich, denn, doch, bloß
	...
	if lang == 'en' or lang == 'ru':
		for w in tree:
			if 'VerbForm=Fin' in w[5]:
				fins += 1
	if lang == 'de':
		# TODO in relative clauses finites are tagged Part: winzige Samenzelle ..., die die Umkehrung des Membranpotentials beansprucht .
		# to fix that test whether the "Part" has a dependent nsubj and has no dependent AUX
		for w in tree:
			kids_lem = get_kids_lem(w, tree)
						...
						try:
							if 'Case=Ins' in tree[i][5] and tree[i][2] not in ['образ', 'лето', 'осень', 'зима',
								...
								if kids_pos and not 'ADP' in kids_pos:
									...
									'''
									# TODO this can be moved up to reduce complexity
