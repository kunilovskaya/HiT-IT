...
original extractors and searchlists are replaced with the versions revised for LREC
The script outputs a tsv with values for translationese features (columns) for each text (rows) + a class value column (eg. good, bad) + language column
-- the language index is taken from the last folder name and needs to be set up accordingly
-- the dict is written to a tsv

folder names are used to produce metadata!:

Usage (the data path and output filename is hardcoded, sorry!)
python3 HTQ_mega_collector.py
'''
import csv
from extractors import *

rootdir = "/home/u2/proj/done/HiT-IT/data/"
data = rootdir + 'pro-ref' # name of the project
outname = rootdir + 'pro385-ref375.tsv'

# here, for each file we collect counts averaged over number of words or number of sentences
## muted features: bypassives shortpassives interrog andor wdlength mark nn
keys = 'afile alang astatus ' \
       ...
       'passives sconj addit advers caus tempseq epist but comp ' \
...
## DONE July 2019   infs partconv nominals shortpassives bypassives connectives
basic_stats = {}
languages = ['en', 'ru']  # 'de'
adv_support = {}
mpred_support = {}
...
for l in languages:
    # import all the lists for SL and TL and add them to a lang-dictionary with lang-keys
...
for subdir, dirs, files in os.walk(data):
    for i, file in enumerate(files):
        ...
        # data hierachy: /home/u2/proj/done/HiT-IT/data/good-bad/bad/ru/*.conllu
        
        # this collects counts for every sentence in a document
        # prepare for writing metadata:
        lang, group = get_HTQmeta(last_folder)
        meta_str = '_'.join(get_HTQmeta(last_folder))  # lang, group for quality
        ...
        current['afile'] = doc
        current['alang'] = lang
        current['astatus'] = group
        
        # ышо раз переливаем, теперь в общий словарь для всех файлов корпуса
        for key in master_dict.keys():
            master_dict[key].append(current[key])
    
    # keys = sorted(master_dict.keys())
    # with open('/home/masha/02targets_wlv/'+'ol_in_23features_May19_rncP400random'+'.tsv', "w") as outfile:
    with open(outname, "w") as outfile:
        
        writer = csv.writer(outfile, delimiter="\t")
        writer.writerow(keys)
        # do I need an asterisk here? it is zipping lists of values for each key from each file
        # empirically no * gets lists of each key values as a list too
        writer.writerows(zip(*[master_dict[key] for key in keys]))

## This is to update the Basic parameters table in the paper
# for k, v in basic_stats.items():
# 	print(k, v)
print('Your bigtable is ready. Lets see whether we can see any patterns in it')
