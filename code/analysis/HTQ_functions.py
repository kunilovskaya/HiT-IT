...
def HTQ_get_xy(df, class_col='astatus', features=15, scaling=1, select_mode='RFE'):
    ...
    training_set = df0.drop(['afile', 'alang', 'astatus'], 1)
    ...
    if features:
        ...
        metadata_df = df.loc[:, ['afile', 'alang', 'astatus']]
...
def HTQ_pca_transform(x, df, dims=2, best='Dim1', print_best=0):
    ...
    try:
        df0 = df.drop(['afile', 'alang', 'astatus'], 1)
    except:
        df0 = df
    if dims == 2:
...
def crossvalidate(X, y, algo='SVM', grid=0, cv=10, class_weight='balanced'):
    ...
    if algo == 'RF' and grid == True:
        ...
        scoring = 'f1_macro'
        print('Running 5-CV gridsearch for RandomForests optimized for %s' % refit_score)
    ...
    for k,v in report.items():
        if k in set(y):
            names.append(k)
            instances.append(v['support'])
    minority_name = names[np.argmin(instances)]
    print('Average F1 on the minority class (%s/%d): %.3f' % (minority_name, report[minority_name]['support'], report[minority_name]['f1-score']))
...
def HTQ_textsdensity(x, y, dimx=1, feats=10):
    ...
    if len(set(y)) == 3:
        ...
        sns.distplot(d['sources'], color="grey", hist=False, rug=True, ax=ax, label='sources',
                     kde_kws={'linestyle': '--'})
        sns.distplot(d['good'], color="C2", hist=False, rug=True, ax=ax, label='good')
        sns.distplot(d['bad'], color="red", hist=False, rug=True, ax=ax, label='bad')
    ...
    if len(set(y)) == 4:
        ...
        sns.distplot(d['sources'], color="orange", hist=False, rug=True, ax=ax, label='sources',
                     kde_kws={'linestyle': '--'})
        sns.distplot(d['good'], color="C2", hist=False, rug=True, ax=ax, label='good')
        sns.distplot(d['bad'], color="red", hist=False, rug=True, ax=ax, label='bad')
